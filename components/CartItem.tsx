  ...
  const handleRemove = (id, resName, resImage) => {
    ...
    <div className="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
      {!!!allCartItems?.length && (
        ...
        <h1 className="text-3xl lg:text-4xl font-semibold leading-7 lg:leading-9  text-gray-800"> Nenhum item do carrinho!</h1>
        <p className="text-base font-medium leading-6 text-gray-600">21st Mart 2021 at 10:34 PM</p>
    </div>
      )}
      {allCartItems?.map((item) => (
            <><div key={item.resName} className="flex justify-start item-start space-y-2 flex-col ">
          <h1 className="text-3xl lg:text-4xl font-semibold leading-7 lg:leading-9  text-gray-800"> {item.resName}</h1>
          <p className="text-base font-medium leading-6 text-gray-600">21st Mart 2021 at 10:34 PM</p>
        </div><div className="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch  w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
              ...
              <div className="flex flex-col justify-start items-start bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
                ...
                <div className="mt-4 md:mt-6 flex  flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full ">
                  <div className="pb-4 md:pb-8 w-full md:w-40">
                    <img className="w-full hidden md:block" src={food.image} alt={food.name}/>
                    <img className="w-full md:hidden" src={food.image} alt={food.name} />
                  </div>
                  <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full  pb-8 space-y-4 md:space-y-0">
                    <div className="w-full flex flex-col justify-start items-start space-y-8">
                      <h3 className="text-xl xl:text-2xl font-semibold leading-6 text-gray-800">{food.name}</h3>
                    ...
                    <div className="flex justify-between space-x-8 items-start w-full">
                      ...
                      {food.price} Kz <span className="text-red-300 line-through"> {food.price} Kz</span>
                      </p>
                      <p className="text-base xl:text-lg leading-6 text-gray-800">{food.quantity} </p>
                      <p className="text-base xl:text-lg font-semibold leading-6 text-gray-800">{food.price} Kz</p>
                    </div>
                  </div>
                </div>
                ))}
